      ...
      <div class="food_cont">
        ...
        <div class="content pr">
          ...
          <div class="cartControl_wrap">
            <transition name="hide">
              <div class="addcartBtn" v-show="!food.count" @click="cartAdd">加入购物车</div>
            </transition>
          </div>
        ...
        <div class="ratings">
          <div class="rate_header">
            ...
            <div class="tip" @click="checkedCont" :class="{checked:checked}"><i class="iconfont icon-radio"></i>只看有内容的评价
            </div>
          </div>
          <div class="ratings_wrap">
            <ul class="rating_list" v-if="food.ratings && food.ratings.length">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      cartAdd(el) {
        this.$emit('food-detail-cart-add', el);
      },
      cartCut() {
        this.$emit('food-detail-cart-cut');
      },
//        addcartBtn(event){
...
<style scoped>
  ...
  .foodDetail .sell_status {
    font-size: 10px;
    color: rgb(147, 153, 159);
    line-height: 20px;
  }

  .foodDetail .sell_status .count {
    margin-right: 12px;
  }
  ...
  .foodDetail .price .now_price em {
    font-size: 10px;
  }

  .foodDetail .price .old_price {
    font-size: 10px;
    color: rgb(147, 153, 159)
  }

  .foodDetail .cartControl_wrap {
    position: absolute;
    right: 18px;
    bottom: 18px;
    z-index: 3;
  ...
  .foodDetail .intro .title, .foodDetail .rate_header .title {
    font-size: 14px;
    font-weight: 600;
    line-height: 14px;
    margin-bottom: 6px;
  ...
  .foodDetail .intro .txt {
    font-size: 12px;
    line-height: 24px;
    color: rgb(77, 85, 93);
  }

  .foodDetail .ratings .rate_header {
    padding: 16px 16px 0;
  }

  .foodDetail .ratings .rateType {
    margin-top: 12px;
    margin-bottom: 18px;;
  }

  .foodDetail .ratings .rateType .type {
    padding: 8px 12px;
    margin-right: 8px;
    color: rgb(77, 85, 93);
    background: rgb(0, 160, 220, .2);
    font-size: 12px;
    border-radius: 2px;
  }

  .foodDetail .ratings .rateType .type.nolike {
    color: rgb(77, 85, 93);
    background: rgb(77, 85, 93, .2)
  }

  .foodDetail .ratings .rateType .type.active {
    background: rgb(0, 160, 240);
    color: #fff;
  }

  .foodDetail .ratings .rateType .type.nolike.active {
    background: rgb(77, 85, 93)
  }

  .foodDetail .ratings .tip {
    font-size: 12px;
    color: rgb(147, 153, 159);
    line-height: 24px;
  ...
  .foodDetail .ratings .tip .iconfont {
    font-size: 18px;
    margin-right: 4px;
    position: relative;
    top: -2px;
  ...
  .foodDetail .ratings .checked .iconfont {
    color: #00c850;
  }

  .foodDetail .rating_list {
    padding: 0 18px;
    border-top: 1px solid rgba(7, 17, 27, .2);
  }

  .foodDetail .rating_list .rating {
    padding: 16px 0;
    position: relative;
    border-bottom: 1px solid rgb(7, 17, 27, .1);
  }

  .foodDetail .rating_list .rating:last-of-type {
    border: none;
  }

  .foodDetail .rating_list .user {
    position: absolute;
    right: 0;
    top: 16px;
    font-size: 10px;
  ...
  .foodDetail .rating_list .avatar {
    border-radius: 50%;
    margin-left: 6px;
    vertical-align: middle;;
  }

  .foodDetail .rating_list .time {
    font-size: 10px;
    color: rgb(147, 153, 159);
    line-height: 12px;
    margin-bottom: 6px;
  ...
  .foodDetail .rating_list .text {
    line-height: 16px;
    color: rgb(7, 17, 27);
    font-size: 12px;
  }

  .foodDetail .rating_list .text .iconfont {
    color: rgb(0, 160, 220);
    line-height: 24px;
    position: relative;
    top: -2px;
  ...
  .foodDetail .rating_list .text .icon-nolike {
    color: rgb(147, 153, 159)
  }
</style>
